 ...
 *      `ICoreWebView2NavigationStartingEventHandler`），所以 `RequestInterceptor`
 *      看不到子资源、也拿不到 `Range` 这类请求头。引擎本身完全支持，只要自己接
 *      `add_WebResourceRequested` + `AddWebResourceRequestedFilter(L"*", ALL)`。
 ...
 *     `SetParent` + 改 `WS_CHILD` 挂进去，尺寸由 `fitViewToParent` 按父窗口客户区
 *     对齐 —— 完全不依赖任何 UI 框架的原生互操作层。
 *
 * 结构（照 cxx/quickjs/quickjs.cpp 的路子，直接编成 SHARED 库、导出 JNI）：
 *
 *   1. 一条**专用的 WebView 线程**，`CoInitializeEx(APARTMENTTHREADED)` 后建一个
 *      永不显示的顶层窗口当消息泵宿主，再在同一线程上建
 ...
 *
 *   3. WebView2 的事件回调**一律不直接干活**，而是 `postTask` 出去再干 —— 在 COM
 *      回调里 `Close()` controller 是文档明确不支持的（可能死锁）。唯一的例外是
 *      请求拦截回调，它必须**同步**给出结论（见 RequestHandler::Invoke）。
 *
 * WebView2Loader.dll 不引入 import lib，运行时按「已加载 → 本 DLL 同目录 → 裸名字」
 * 的顺序 `LoadLibrary`。JNI 库是被 `System.load` 到临时目录的，那个目录不在任何
 * 默认搜索路径里，所以「本 DLL 同目录」这一档是必需的。
 */
...
// 头文件在 $JAVA_HOME/include{,/win32}（cxx/CMakeLists.txt 已经加进 include 路径），
// 但**不链接** jawt.lib —— 和 WebView2Loader 一样运行时 LoadLibrary。
...
//
// 两个都开就两份都写。为什么要文件：这是个 native DLL，在 GUI 程序里 stderr 没人
// 接；而且 JVM 侧 `System.load` 进来的库和宿主各有各的 CRT，stderr 不一定是同一个
// 句柄，日志很容易「写了但看不到」。
// ---------------------------------------------------------------------------
static FILE* logFile() {
    static FILE* f = [] () -> FILE* {
        char path[MAX_PATH]{};
        if (GetEnvironmentVariableA("ACG_WEBVIEW_LOG", path, sizeof(path)) == 0) return nullptr;
...
static HMODULE selfModule() {
    ...
    if (VirtualQuery(reinterpret_cast<LPCVOID>(&selfModule), &mbi, sizeof(mbi)) == 0) return nullptr;
...
static bool ensureLoader() {
        ...
        if (HMODULE self = selfModule()) {
            ...
            if (n > 0 && n < MAX_PATH * 2) {
                ...
                if (slash != std::wstring::npos) {
                    const std::wstring cand = p.substr(0, slash + 1) + L"WebView2Loader.dll";
                    g_loader = LoadLibraryExW(cand.c_str(), nullptr, LOAD_WITH_ALTERED_SEARCH_PATH);
    ...
    g_pfnCreateEnv = reinterpret_cast<PFN_CreateEnvWithOptions>(
        reinterpret_cast<void*>(GetProcAddress(g_loader, "CreateCoreWebView2EnvironmentWithOptions")));
    g_pfnGetVersion = reinterpret_cast<PFN_GetBrowserVersion>(
        reinterpret_cast<void*>(GetProcAddress(g_loader, "GetAvailableCoreWebView2BrowserVersionString")));
    if (!g_pfnCreateEnv) {
...
// 初始化失败的诊断信息
//
// 思路来自 wvbridge 的 `libs_helpers.cpp::build_init_error`。光给一个 HRESULT 数字
// 在排查时几乎没用 —— 我们之前就吃过亏：环境创建失败只打出一句
// 「创建 WebView2 环境失败（hr=0x80070005）」，看不出到底是「没装运行时」
// 「user data folder 没权限」还是「运行时自己启动崩了」。
//
...
static std::wstring hresultName(HRESULT hr) {
    switch (hr) {
        ...
        case HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND): return L"ERROR_FILE_NOT_FOUND";
        case HRESULT_FROM_WIN32(ERROR_PATH_NOT_FOUND): return L"ERROR_PATH_NOT_FOUND";
        case HRESULT_FROM_WIN32(ERROR_MOD_NOT_FOUND): return L"ERROR_MOD_NOT_FOUND";
        case HRESULT_FROM_WIN32(ERROR_PROC_NOT_FOUND): return L"ERROR_PROC_NOT_FOUND";
        case HRESULT_FROM_WIN32(ERROR_SHARING_VIOLATION): return L"ERROR_SHARING_VIOLATION";
        case HRESULT_FROM_WIN32(ERROR_BUSY): return L"ERROR_BUSY";
        case HRESULT_FROM_WIN32(ERROR_TIMEOUT): return L"ERROR_TIMEOUT";
        default: return {};
    }
}
...
static const wchar_t* hintOf(HRESULT hr) {
    if (hr == HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND)) {
        return L"找不到可用的 WebView2 运行时 —— 装 Evergreen Runtime，或确认 WebView2Loader.dll 在位";
    }
    if (hr == E_ACCESSDENIED || hr == HRESULT_FROM_WIN32(ERROR_ACCESS_DENIED)) {
        return L"WebView2 读不到 user data folder —— 换个可写目录，并别去动它的 ACL";
    }
    if (hr == HRESULT_FROM_WIN32(ERROR_SHARING_VIOLATION) || hr == HRESULT_FROM_WIN32(ERROR_BUSY)) {
        return L"user data folder 被别的进程占着 —— 同一个目录只能有一个进程的 WebView2 用它";
    }
    if (hr == E_FAIL) {
        return L"运行时起来又失败了 —— 看 user data folder 下的 EBWebView/chrome_debug.log 里浏览器进程为什么 CHECK";
    }
    return nullptr;
}
 ...
 */
static std::wstring describeEnvFailure(const wchar_t* stage, HRESULT hr, const std::wstring& userDataDir) {
    wchar_t buf[64]{};
...
//   宿主这边只看到「WebView2 进程异常退出（kind=0 reason=0）」—— 除了 `about:blank`
//   （不需要合成器）之外的任何页面都打不开，连 `NavigationStarting` 都不触发。
//
//   加上这个开关之后 GPU 进程照样崩，但**不再拖垮浏览器进程**：三个端到端用例
//   从 0/3 变成 2/3（见 `shared/src/jvmTest/.../NativeWebViewHostTest.kt`）。
//
//   注意区别：`--disable-gpu` **没用**（试过）—— 它只是关掉硬件加速，GPU 进程照样
//   会启动、照样会崩、照样会把浏览器进程带走。真正要改的是「崩了别自杀」这条策略。
//
// 为什么走进程环境变量而不是 `ICoreWebView2EnvironmentOptions`：选项接口要手写一套
// COM 类（MinGW 的 WRL 没有 `RuntimeClass`，见下文 ComCallback 那段的说明），而
// `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` 是 WebView2Loader 在创建环境时直接读的，
// 浏览器子进程又从我们这儿继承环境块 —— 效果一样，代码只有几行。
// 已经设过的值会被**追加**而不是覆盖，方便临时替换参数做对照。
//
// **这台机器上「子进程崩溃」这件事的实测结论（2026-09-15，别再来回试）**：
//
//   症状：GPU(6)/Utility(4) 进程以 `reason=2`(`TERMINATED`) `exit=1` 无限重启；
//         一场 run 里几百到几千条。页面 `NavigationCompleted success=1` 但白屏。
//   对照（同一份代码、同一份 user data folder，只换启动方式/参数）：
//     | 参数 / 启动方式                    | 进程失败条数 | 页面 | 按键进页面 |
//     | 仅 `--disable-gpu-process-crash-limit`（本函数加的）| 623~4870 | 白屏 | 否 |
//     | 再加 `--disable-gpu-sandbox`       | 177~186（只剩 kind=4）| 正常 | 否 |
//     | 再加 `--no-sandbox`                | **0** | 正常 | **是** |
...
//
//   **而「干净启动」这一行说明这不是本工程的缺陷**：从 `explorer.exe`（脱离 agent
//   那条进程树）启动、一个沙箱参数都不给，进程失败就是 0，App 完全正常。
//   排除过的常见元凶：Exploit Protection 缓解项（没有）、IFEO `Debugger` 劫持（没有）、
//   AppInit_DLLs（0）、user data folder 位置（`%LOCALAPPDATA%\ACG\webview2`，NTFS
//   正常 ACL）、第三方杀软（只有 Windows Defender）。
//
...
//   `CreateCoreWebView2Controller` 不动），所以**别把偶发现象写成代码里的结论**。
// ---------------------------------------------------------------------------
...
static void applyDefaultBrowserArguments() {
    ...
    wchar_t buf[2048]{};
    const DWORD n = GetEnvironmentVariableW(key, buf, sizeof(buf) / sizeof(buf[0]));
    ...
    if (SetEnvironmentVariableW(key, value.c_str())) {
        ...
        WV_LOG("SetEnvironmentVariableW 失败 (%lu)，浏览器参数没生效", GetLastError());
    }
}
...
// 手写的 COM 回调基类
//
// MinGW 的 WRL 只移植了 ComPtr（wrl.h 里 implements.h / event.h 都是注释掉的），
// 既没有 `Microsoft::WRL::RuntimeClass` / `Callback<>`，也没有 `__uuidof`。
// 所以自己实现引用计数 + QueryInterface，IID 由子类显式传入。
// ---------------------------------------------------------------------------
template <class I>
...
// 把宿主窗口 `SetParent` 到那个组件的 HWND。JAWT 是官方给的路子，wvbridge 也用它。
//
// 不链接 `jawt.lib`：MinGW 编 MSVC 的 import lib 很麻烦，而 `jawt.dll` 在
// `$JAVA_HOME/bin`、JVM 起来之后本来就已经映射进本进程了 —— 直接
// `GetModuleHandleW` / `LoadLibraryW` 取 `JAWT_GetAWT` 就行，和 WebView2Loader 一个套路。
// ---------------------------------------------------------------------------
...
/**
 * 取 AWT 组件的原生窗口句柄；非 Windows 组件 / 还没 peer / JAWT 不可用都返回 nullptr。
 *
...
static HWND awtHwndOf(JNIEnv* env, jobject component) {
    ...
    if ((lock & JAWT_LOCK_ERROR) == 0) {
        if (JAWT_DrawingSurfaceInfo* dsi = ds->GetDrawingSurfaceInfo(ds)) {
            if (auto* info = static_cast<JAWT_Win32DrawingSurfaceInfo*>(dsi->platformInfo)) {
                hwnd = info->hwnd;
            }
...
struct View {
     ...
     * 以 `GetClientRect(parent)` 为准（见 [fitViewToParent]）。后台视图恒为 null。
     */
    HWND parent = nullptr;
     ...
     * 见 [ensureThreadAttached] —— 这是「能点不能打字」的最终解，别再改回临时接法。
     */
    DWORD attachedTid = 0;
};
...
static std::shared_ptr<View> findBackgroundByToken(jlong token) {
    ...
    for (const auto& entry : g_views) {
        if (entry.second->background && entry.second->token == token) return entry.second;
    }
    return nullptr;
}
...
/**
 * 把 WebView 线程**长久**接到 AWT 线程（根窗口 `SunAwtFrame` 所在线程）的输入队列上。
 *
 * ## 为什么必须「长久」，临时接一下不够
 *
 * 键盘是**按前台窗口所在线程的 `hwndFocus`** 路由的。前台窗口是 AWT 的
 * `SunAwtFrame`，宿主 `AcgNativeWebViewHost` 建在 WebView 线程上 —— 两条输入队列
 * 各自记一份焦点：
 *
 * ```
 * 前景窗口 = SunAwtFrame
 ...
 *
 * 长久接上之后两条线程共享同一份输入状态，`MoveFocus` / `SetFocus` 的结果**两条队列
 * 都认**，前台那条也就跟着指向 `Chrome_WidgetWin_1`，键盘才真的进渲染器。
 *
 * ## 为什么接「根窗口的线程」，不是「前台窗口的线程」
 *
 * 前台可能是**别人家 App** 的窗口；把我们的队列长久接到一个外部进程的线程上，
 * 后果不可控。根窗口（`GetAncestor(host, GA_ROOT)` = `SunAwtFrame`）永远是我们自己
 * 的，同进程、同生命周期，安全。
 *
 * ## 副作用要有数
 *
 ...
 */
/**
 * 长久接队列的开关。设 `ACG_WEBVIEW_NO_THREAD_ATTACH=1` 关掉（退回「只在自己线程里
 * 设焦点」的老行为 —— 焦点日志看着正常，但键盘进不了页面）。
 *
 * 留这个口子是因为 `AttachThreadInput` 会让两条消息循环在 `SendMessage` 层面互相牵连，
 * 万一在别的机器上出现界面卡死，可以**不重编**就直接关掉对比。
 * 环境变量只读一次（进程内缓存）。
 */
static bool threadInputAttachEnabled() {
    ...
    if (enabled < 0) {
        char buf[16]{};
        const DWORD n = GetEnvironmentVariableA("ACG_WEBVIEW_NO_THREAD_ATTACH", buf, sizeof(buf));
        enabled = (n > 0 && buf[0] != '0' && buf[0] != '\0') ? 0 : 1;
    }
    return enabled == 1;
}

static void ensureThreadAttached(const std::shared_ptr<View>& view, HWND host, DWORD hostTid) {
    ...
    if (AttachThreadInput(hostTid, rootTid, TRUE)) {
        ...
        WV_LOG("view %p: 输入队列已接到 AWT 线程 (本线程 %lu -> %lu)", host, hostTid, rootTid);
    } else {
        WV_LOG("view %p: AttachThreadInput 失败 (本线程 %lu -> %lu) err=%lu", host, hostTid,
               rootTid, GetLastError());
    }
}
...
static void detachThreadInput(const std::shared_ptr<View>& view, HWND host) {
    ...
    WV_LOG("view %p: 输入队列已摘开 (本线程 %lu <- %lu)", host, hostTid, view->attachedTid);
    view->attachedTid = 0;
}
...
// 'ScopedThreadInput'（临时接一下、析构就摘）已于 2026-09-15 删除：它只能让我们自己
// 的 `GetFocus()` 变对，改不动**前台队列**的 `hwndFocus`，键盘照样被 `SunAwtFrame`
// 吃掉。取而代之的是 [ensureThreadAttached] 的**长久**接法。
 ...
 * 判据的三个细节：
 *
 * - **要问「宿主所在线程」，不是「前台窗口所在线程」。** 宿主窗口建在 WebView 线程上，
 *   焦点真送进去之后 `GetGUIThreadInfo(宿主tid).hwndFocus` 就是
 *   `Chrome_WidgetWin_1`（探针实测）。反过来问前台线程是**错的**：前台恒为 AWT 的
 *   `SunAwtFrame`，而那条队列的 `hwndFocus` 在焦点进 WebView2 之后竟然是**空的**
 *   （探针实测 `tid=<awt> hwndFocus=(空)`）—— 拿它判会永远 false，闸门等于不存在，
 *   于是每次页面喊一声就重新 `MoveFocus`，把输入法组合状态反复打断。
 * - **`GetFocus()` 在我们自己线程上就够用**：本函数一直在 WebView 线程上跑，
 *   `GetFocus()` 读的正是这条队列，跟 `GetGUIThreadInfo(宿主tid).hwndFocus` 等价。
 *   万一将来从别的线程调进来，退化成 `GetGUIThreadInfo(宿主tid)` 也行。
 * - **宿主的直接子窗口不算**：`Chrome_WidgetWin_0` 只是 WebView2 的中间层，焦点停在
 *   那儿键盘进不了渲染器，仍然需要 `MoveFocus`。真正算数的是它下面的
 *   `Chrome_WidgetWin_1` / `Chrome_RenderWidgetHostHWND`。
 * - 宿主自己（`AcgNativeWebViewHost`）拿到焦点同理不算 —— 那正是「需要转交」的场景。
 *
 * 另外还要**前台确实是我们这棵窗口树**：切到别的 App 之后本线程队列可能残留着旧的
...
static bool webViewAlreadyHasKeyboardFocus(HWND host) {
    ...
    if (GetWindowThreadProcessId(host, nullptr) == GetCurrentThreadId()) {
        ...
        if (GetGUIThreadInfo(GetWindowThreadProcessId(host, nullptr), &gti)) f = gti.hwndFocus;
    }
...
/**
 * 把键盘焦点送进 WebView2。**这是「能点不能打字」的唯一解**，细节都是探针实测出来的。
 *
 * ## 为什么必须显式做
 *
 * 宿主的 HWND 拿到焦点**不会自动传给** WebView2 的子窗口，而 WebView2 自己也不抢焦点：
 ...
 *
 *     host(本线程) → Chrome_WidgetWin_0(本线程) → Chrome_WidgetWin_1(浏览器进程)
 ...
 * ## 关键实现细节（都是踩出来的）
 *
 * - **`MoveFocus(PROGRAMMATIC)` 是主路，但单独用它不够。** 探针（宿主就是顶层窗口、
 *   进程里只有它一个窗口）实测：调完 `GetFocus()` 变成 `Chrome_WidgetWin_1`，页面
 *   `document.hasFocus()` 从 0 变 1，敲键页面里的 `keydown` 计数器真的在涨。
 *   **但那只是因为探针的宿主窗口自己就是前台窗口**；真正嵌进 AWT 之后不成立 ——
 *   前台是 AWT 线程的 `SunAwtFrame`，而宿主窗口建在 WebView 线程上，两个输入队列
 ...
 * - **`GetFocus()` 是「按线程消息队列」算的**：本函数跑在 WebView 线程上，若焦点在
 *   AWT 线程的队列里，这里读到的就是 NULL —— 正好可以当作「不在我们这棵树上」。
 * - **`SetFocus` 只能点名本线程队列里的窗口**，而 `Chrome_WidgetWin_0` 恰好就是建在
 *   我们宿主（WebView 线程）下面的，所以下面那个兜底可以放心点。
 * - **要重试**：AWT / Compose 处理激活流程时可能**在我们之后**才 `SetFocus` 到自己的
...
static void requestWebViewFocus(HWND host, int retries) {
    ...
    // **幂等**：焦点已经在 WebView 的输入窗口上就什么都不做，连重试定时器都不再挂。
    //
    ...
    // 第一步：确保前台是**我们自己的窗口**。不是的话先把它顶上去 —— 后台进程抢焦点
    // 系统本来就会拒，后面 SetFocus 更没有意义。顺序依旧是「先顶前台，再 MoveFocus」，
    // 反过来会把刚送进 WebView 的焦点又拽回来。
    ...
    if (ok) {
        WV_LOG("view %p: 焦点进入 WebView (focus=%p)", host, static_cast<void*>(GetFocus()));
    } else {
        ...
        WV_LOG("view %p: 焦点未进入 WebView (focus=%p 前台队列焦点=%p target=%p err=%lu "
               "前台=%p/%lu 宿主tid=%lu 本tid=%lu 已接=%lu) 剩余重试=%d",
               host, static_cast<void*>(GetFocus()), static_cast<void*>(fgFocus),
               static_cast<void*>(target), GetLastError(), static_cast<void*>(fg), fgTid, hostTid,
               myTid, static_cast<unsigned long>(view->attachedTid), retries);
    }
    if (retries > 0) {
...
/** 延后一点把键盘焦点送进 WebView（激活流程还没走完时 SetFocus 可能被系统拒绝）。 */
 ...
 *
 * 必须是原子的：JVM 线程随时在读它投任务，而 WebView 线程在退出路径上会把它清掉。
 * 裸 HWND 在这里就是数据竞争（而且清完之后 JVM 线程可能拿着一个已经销毁的句柄
 * `PostMessageW`）。
 */
...
/** 同步地在 WebView 线程上跑一个闭包并等它回来（只用于「建窗口」这种极短操作）。 */
template <class Fn>
static bool runSync(const Fn& fn, int timeoutMs) {
    ...
    return box->cv.wait_for(lock, std::chrono::milliseconds(timeoutMs), [&] { return box->done; });
}
...
static void notifyBackgroundFinished(jlong token, const std::wstring& json, bool hasJson) {
    ...
    if (g_cb && g_midFinished) env->CallVoidMethod(g_cb, g_midFinished, token, arg);
...
static void notifyViewEvent(HWND hwnd, ViewEvent what, const std::wstring& a, const std::wstring& b) {
    JNIEnv* env = envForThread();
    if (!env) return;
    ...
    if (g_cb && g_midViewEvent)
        env->CallVoidMethod(g_cb, g_midViewEvent, static_cast<jlong>(reinterpret_cast<intptr_t>(hwnd)),
...
static void notifyScriptResult(HWND hwnd, jlong token, const std::wstring& json, bool hasJson) {
    JNIEnv* env = envForThread();
    if (!env) return;
    jstring ja = hasJson ? fromWide(env, json) : nullptr;
    std::lock_guard<std::mutex> lock(g_cbMutex);
    if (g_cb && g_midScriptResult)
        env->CallVoidMethod(g_cb, g_midScriptResult, static_cast<jlong>(reinterpret_cast<intptr_t>(hwnd)),
                            token, ja);
    if (env->ExceptionCheck()) {
        env->ExceptionDescribe();
        env->ExceptionClear();
...
static void destroyView(HWND hwnd) {
    ...
    view->destroyed = true;
    WV_LOG("view %p: destroy 开始 (background=%d)", hwnd, view->background ? 1 : 0);
    if (view->webview) {
        if (view->requestHandler) view->webview->remove_WebResourceRequested(view->tRequest);
        if (view->navStartHandler) view->webview->remove_NavigationStarting(view->tNavStart);
        if (view->navDoneHandler) view->webview->remove_NavigationCompleted(view->tNavDone);
        if (view->sourceHandler) view->webview->remove_SourceChanged(view->tSource);
        if (view->historyHandler) view->webview->remove_HistoryChanged(view->tHistory);
        if (view->titleHandler) view->webview->remove_DocumentTitleChanged(view->tTitle);
        if (view->newWindowHandler) view->webview->remove_NewWindowRequested(view->tNewWindow);
        if (view->processFailedHandler) view->webview->remove_ProcessFailed(view->tProcessFailed);
    }
    view->webview.Reset();
 ...
 * 一样在后台线程上同步返回。
 *
 * 用 `GetDeferral()` 异步化需要把 commonMain 的 `WebViewTask.onInterceptRequest`
 * 改成 suspend，牵动面太大，不做。
 */
class RequestHandler final : public ComCallback<ICoreWebView2WebResourceRequestedEventHandler> {
public:
    RequestHandler(HWND hwnd, jlong token, ICoreWebView2Environment* env)
        : ComCallback(IID_ICoreWebView2WebResourceRequestedEventHandler), hwnd_(hwnd), token_(token), env_(env) {
        if (env_) env_->AddRef();
    }
    ...
    HRESULT STDMETHODCALLTYPE Invoke(ICoreWebView2* sender,
        ...
        WV_LOG("view %p: WebResourceRequested %ls ctx=%d token=%lld", hwnd_, uri.c_str(),
               static_cast<int>(ctx), static_cast<long long>(token_));
        ...
        jobjectArray jHeaders = env->NewObjectArray(static_cast<jsize>(flat.size()),
        ...
        jboolean hit = JNI_FALSE;
        {
            ...
            if (g_cb && g_midIntercept)
                hit = env->CallBooleanMethod(g_cb, g_midIntercept, token_, jUri, jMethod,
        ...
        if (env_ && SUCCEEDED(env_->CreateWebResourceResponse(nullptr, 204, L"No Content", L"", response.GetAddressOf())) && response) {
...
public:
    explicit NavigationStartingHandler(HWND hwnd)
        : ComCallback(IID_ICoreWebView2NavigationStartingEventHandler), hwnd_(hwnd) {}
    HRESULT STDMETHODCALLTYPE Invoke(ICoreWebView2* sender,
...
class ScriptHandler final : public ComCallback<ICoreWebView2ExecuteScriptCompletedHandler> {
public:
    ScriptHandler(HWND hwnd, jlong token)
        : ComCallback(IID_ICoreWebView2ExecuteScriptCompletedHandler), hwnd_(hwnd), token_(token) {}
...
public:
    explicit NavigationCompletedHandler(HWND hwnd)
        : ComCallback(IID_ICoreWebView2NavigationCompletedEventHandler), hwnd_(hwnd) {}
...
private:
    static void onDone(HWND hwnd, bool success, COREWEBVIEW2_WEB_ERROR_STATUS err) {
        std::shared_ptr<View> view = findView(hwnd);
        WV_LOG("view %p: NavigationCompleted success=%d err=%d background=%d", hwnd, success ? 1 : 0,
               static_cast<int>(err), view && view->background ? 1 : 0);
        if (!view) return;
        notifyViewEvent(hwnd, kViewLoading, L"0", L"");

        if (view->settled || view->notified) return;
        // 我们自己在拦截后调的 Stop() 会引出一个 OperationCanceled，静默吞掉。
        if (!success && err == COREWEBVIEW2_WEB_ERROR_STATUS_OPERATION_CANCELED) return;

        if (!view->background) return;

        if (!success) {
            notifyBackgroundFailed(view->token, L"导航失败（WebView2 错误码 " +
                                                   std::to_wstring(static_cast<int>(err)) + L"）：" + view->url);
            return;
        }
        if (!view->hasScript) {
        ...
        const HRESULT hr = view->webview->ExecuteScript(view->script.c_str(), handler);
        handler->Release();
        if (FAILED(hr)) notifyBackgroundFailed(view->token, L"ExecuteScript 调用失败");
    }
...
/** 文档地址变了。 */
class SourceChangedHandler final : public ComCallback<ICoreWebView2SourceChangedEventHandler> {
public:
    ...
    HRESULT STDMETHODCALLTYPE Invoke(ICoreWebView2* sender,
        ...
        const std::wstring url = takeString([&](LPWSTR* out) { return wv->get_Source(out); });
        postTask([hwnd, url] {
...
/** 前进/后退栈变化。 */
class HistoryChangedHandler final : public ComCallback<ICoreWebView2HistoryChangedEventHandler> {
public:
    ...
    HRESULT STDMETHODCALLTYPE Invoke(ICoreWebView2* sender, IUnknown* args) override {
        ...
        postTask([hwnd, back = back != FALSE, forward = forward != FALSE] {
            if (findView(hwnd))
                notifyViewEvent(hwnd, kViewHistory, back ? L"1" : L"0", forward ? L"1" : L"0");
        });
        return S_OK;
    }
...
/** 标题变化。 */
class TitleChangedHandler final : public ComCallback<ICoreWebView2DocumentTitleChangedEventHandler> {
public:
    explicit TitleChangedHandler(HWND hwnd)
        : ComCallback(IID_ICoreWebView2DocumentTitleChangedEventHandler), hwnd_(hwnd) {}
    HRESULT STDMETHODCALLTYPE Invoke(ICoreWebView2* sender, IUnknown* args) override {
        (void)args;
        const std::wstring title = takeString([&](LPWSTR* out) { return sender->get_DocumentTitle(out); });
        const HWND hwnd = hwnd_;
 ...
 * 以后真需要多窗口，这里是唯一要改的地方。
 */
class NewWindowHandler final : public ComCallback<ICoreWebView2NewWindowRequestedEventHandler> {
public:
    NewWindowHandler(HWND hwnd, bool swallow)
        : ComCallback(IID_ICoreWebView2NewWindowRequestedEventHandler), hwnd_(hwnd),
          swallow_(swallow) {}
 ...
 * （日志里 `reason=2` 就是 `TERMINATED`，`exit=1` 也是这么来的），WebView2 自己会
 * 兜底到软件渲染，页面照常加载。
 *
 ...
 */
class ProcessFailedHandler final : public ComCallback<ICoreWebView2ProcessFailedEventHandler> {
public:
    ...
    HRESULT STDMETHODCALLTYPE Invoke(ICoreWebView2* sender,
        ...
        if (SUCCEEDED(args->QueryInterface(IID_ICoreWebView2ProcessFailedEventArgs2,
        ...
        postTask([hwnd, kind, reason, exitCode] {
            ...
            WV_LOG("view %p: process failed kind=%d reason=%d exit=%d%s", hwnd,
                   ...
                   static_cast<int>(exitCode), isFatalProcessFailure(kind) ? "" : "（非致命，忽略）");
            if (!isFatalProcessFailure(kind)) return;
            const std::wstring message = L"WebView2 进程异常退出（kind=" +
...
static void configureView(const std::shared_ptr<View>& view, const ViewOptions& opt,
                          ICoreWebView2Controller* controller, ICoreWebView2* webview);

/** 控制器就绪后继续：设置项、事件、注册拦截、导航。 */
class ControllerHandler final
    : public ComCallback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler> {
public:
    ControllerHandler(HWND hwnd, ViewOptions opt)
        : ComCallback(IID_ICoreWebView2CreateCoreWebView2ControllerCompletedHandler),
          hwnd_(hwnd), opt_(std::move(opt)) {}

    HRESULT STDMETHODCALLTYPE Invoke(HRESULT errorCode, ICoreWebView2Controller* controller) override {
        const HWND hwnd = hwnd_;
        WV_LOG("view %p: CreateCoreWebView2Controller 回调 hr=0x%08lX controller=%p",
               hwnd, static_cast<unsigned long>(errorCode), static_cast<void*>(controller));
        ...
        if (!webview) {
            const jlong token = opt_.token;
            postTask([hwnd, token] {
                notifyViewEvent(hwnd, kViewFailed, L"控制器没有给出 ICoreWebView2", L"");
...
static void configureView(const std::shared_ptr<View>& view, const ViewOptions& opt,
    ...
    // configureView 整段都跑在 WebView 线程上的单个任务里，提前不会和别的线程打架。
    view->ready = true;
    g_env->QueryInterface(IID_ICoreWebView2Environment2, reinterpret_cast<void**>(view->env2.GetAddressOf()));
    WV_LOG("view %p: configureView background=%d env2=%d", view->hwnd, view->background ? 1 : 0,
           view->env2 ? 1 : 0);
    ...
    if (SUCCEEDED(webview->get_Settings(settings.GetAddressOf())) && settings) {
        ...
        settings->put_AreDefaultContextMenusEnabled(opt.enableDevtools ? TRUE : FALSE);
    }
    if (!opt.userAgent.empty()) {
        ComPtr<ICoreWebView2Settings2> settings2;
        if (SUCCEEDED(webview->QueryInterface(IID_ICoreWebView2Settings2,
    ...
    // 后台视图是隐藏顶层窗口，1x1 就够；可见视图尺寸以 AWT 组件（父窗口）客户区为准。
    if (view->background) {
        RECT bounds{0, 0, 1, 1};
    ...
    // ---- 事件 ----
    auto* reqHandler = new (std::nothrow) RequestHandler(view->hwnd, view->token, g_env.Get());
    if (reqHandler) {
        view->requestHandler = reqHandler;
        reqHandler->Release();
    }
    if (view->requestHandler) {
        const HRESULT hr = webview->add_WebResourceRequested(view->requestHandler.Get(), &view->tRequest);
        WV_LOG("view %p: add_WebResourceRequested hr=0x%08lX token=%lld",
               view->hwnd, static_cast<unsigned long>(hr), static_cast<long long>(view->token));
        if (SUCCEEDED(hr)) {
            // ==== 关键：filter 用 "*" + ALL，子资源（含 Range 分片）才看得见 ====
            const HRESULT hrFilter =
...
#define ADD_HANDLER(field, token, method, type)                                        \
    ...
    ADD_HANDLER(navStartHandler, tNavStart, add_NavigationStarting, NavigationStartingHandler);
    ADD_HANDLER(navDoneHandler, tNavDone, add_NavigationCompleted, NavigationCompletedHandler);
    ADD_HANDLER(sourceHandler, tSource, add_SourceChanged, SourceChangedHandler);
    ADD_HANDLER(historyHandler, tHistory, add_HistoryChanged, HistoryChangedHandler);
    ADD_HANDLER(titleHandler, tTitle, add_DocumentTitleChanged, TitleChangedHandler);
    ADD_HANDLER(processFailedHandler, tProcessFailed, add_ProcessFailed, ProcessFailedHandler);
#undef ADD_HANDLER
    ...
    if (!opt.allowNewWindow) {
        ...
        if (h) {
            ...
            h->Release();
            webview->add_NewWindowRequested(view->newWindowHandler.Get(), &view->tNewWindow);
        }
    }
    ...
    if (view->background) {
        ...
        if (view->env2 && view->headers.size() >= 2 &&
            SUCCEEDED(webview->QueryInterface(IID_ICoreWebView2_2,
            ...
            const HRESULT hrReq = view->env2->CreateWebResourceRequest(
                view->url.c_str(), L"GET", nullptr, headerBlock.c_str(), request.GetAddressOf());
            if (SUCCEEDED(hrReq) && request) {
                const HRESULT hrNav = webview2->NavigateWithWebResourceRequest(request.Get());
                navigated = SUCCEEDED(hrNav);
                WV_LOG("view %p: NavigateWithWebResourceRequest hr=0x%08lX url=%ls", view->hwnd,
                       static_cast<unsigned long>(hrNav), view->url.c_str());
            } else {
                WV_LOG("view %p: CreateWebResourceRequest hr=0x%08lX（退回 Navigate）", view->hwnd,
                       static_cast<unsigned long>(hrReq));
            }
        }
        if (!navigated) {
        ...
        if (view->env2 && !opt.headers.empty() &&
            ...
            if (SUCCEEDED(view->env2->CreateWebResourceRequest(opt.url.c_str(), L"GET", nullptr,
                                                              headerBlock.c_str(), request.GetAddressOf())) &&
                request) {
                navigated = SUCCEEDED(webview2->NavigateWithWebResourceRequest(request.Get()));
            }
        }
        // 可见视图记得初始 URL，供 ResolveNavigation 之类使用（目前不额外做）。
        if (!navigated) navigated = SUCCEEDED(webview->Navigate(opt.url.c_str()));
        if (!navigated) notifyViewEvent(view->hwnd, kViewFailed, L"导航调用失败", L"");
    }
}

/** 建环境（异步）→ 回调里唤醒 nativeStart 的等待。 */
class EnvironmentHandler final
    : public ComCallback<ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler> {
public:
    EnvironmentHandler() : ComCallback(IID_ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler) {}

    HRESULT STDMETHODCALLTYPE Invoke(HRESULT errorCode, ICoreWebView2Environment* env) override {
        WV_LOG("EnvironmentHandler: hr=0x%08lX env=%p", static_cast<unsigned long>(errorCode),
               static_cast<void*>(env));
        // 消息在锁外拼：它要去问 loader 当前运行时是哪个版本。
        const std::wstring failure =
            (SUCCEEDED(errorCode) && env)
                ? std::wstring()
                : describeEnvFailure(L"创建 WebView2 环境失败", errorCode, g_userDataDir);
        {
...
static LRESULT CALLBACK wndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    switch (msg) {
        ...
        // ---- 键盘焦点 ----
        //
        // 注意这里**没有** `WM_PARENTNOTIFY` / `WM_LBUTTONDOWN` 之类的「点击→抢焦点」：
        ...
        case WM_MOUSEACTIVATE:
            // 点在宿主**自己**身上（WebView2 还没铺满、或者页面比宿主小时露出来的那点边）
            // 的情况。**不能**在这里直接 SetFocus —— 激活流程还没走完，系统可能拒绝；
...
static HWND createViewWindow(HWND parent, bool background) {
    const DWORD style = background ? (WS_POPUP) : (WS_POPUP | WS_CLIPCHILDREN | WS_CLIPSIBLINGS);
    return CreateWindowExW(0, kWindowClass, L"", style, 0, 0, 1, 1, parent, nullptr,
...
static void webViewThreadMain() {
    ...
    // 让 PostThreadMessageW 能投到我们身上 —— 消息队列要等第一次 GetMessage/PeekMessage
    // 才存在，但我们马上就要建窗口，所以这里不用像 wvbridge 那样先 PeekMessage 一下。
    ...
    if (!ensureLoader()) {
        ...
        // 必须在建环境**之前**设好 —— loader 是在 CreateCoreWebView2EnvironmentWithOptions
        ...
        if (!handler) {
            ...
            const HRESULT hr = g_pfnCreateEnv(nullptr, dir.empty() ? nullptr : dir.c_str(), nullptr, handler);
            handler->Release();
            if (FAILED(hr))
                failEnvInit(describeEnvFailure(L"CreateCoreWebView2EnvironmentWithOptions 调用失败", hr, dir));
        }
    }
    ...
    // 兜底：正常路径上 nativeStop 已经在泵还活着的时候把视图拆干净了，这里一般是空转。
...
static std::wstring ensureStarted(const std::wstring& userDataDir, int timeoutMs) {
    if (g_thread.joinable()) {
        std::lock_guard<std::mutex> lock(g_envMutex);
        return g_envError;
    }
    if (!ensureLoader()) return L"找不到 WebView2Loader.dll（应和 webview.dll 放在同一目录）";
    const std::wstring version = runtimeVersion();
    if (version.empty())
        return L"系统没有安装 WebView2 运行时（Win11 自带，Win10 需要装 Evergreen Runtime）";
    ...
    if (!g_envCv.wait_for(lock, std::chrono::milliseconds(timeoutMs > 0 ? timeoutMs : 20000),
                          [] { return g_envDone; })) {
...
// ---------------------------------------------------------------------------
#define WV_JNI(ret, name) extern "C" JNIEXPORT ret JNICALL Java_soko_ekibun_acg_web_NativeWebView_##name
#define WV_JNI_PARAMS JNIEnv* env, jobject

/** 从 JVM 的 String[] 读摊平的头。 */
static std::vector<std::wstring> readHeaderArray(JNIEnv* env, jobjectArray headers) {
 ...
 */
WV_JNI(jstring, nativeStart)(JNIEnv* env, jobject, jobject callback, jstring userDataDir, jint timeoutMs) {
    if (g_thread.joinable()) {
        std::lock_guard<std::mutex> lock(g_cbMutex);
    ...
    g_midIntercept = env->GetMethodID(cls, "nativeInterceptRequest",
                                      "(JLjava/lang/String;Ljava/lang/String;Z[Ljava/lang/String;)Z");
    g_midFinished = env->GetMethodID(cls, "nativeFinished", "(JLjava/lang/String;)V");
    g_midFailed = env->GetMethodID(cls, "nativeFailed", "(JLjava/lang/String;)V");
    g_midViewEvent = env->GetMethodID(cls, "nativeViewEvent", "(JILjava/lang/String;Ljava/lang/String;)V");
    g_midScriptResult = env->GetMethodID(cls, "nativeScriptResult", "(JJLjava/lang/String;)V");
    env->DeleteLocalRef(cls);
    if (!g_midIntercept || !g_midFinished || !g_midFailed || !g_midViewEvent || !g_midScriptResult) {
        if (env->ExceptionCheck()) env->ExceptionClear();
        return fromWide(env, L"回调对象缺少 nativeInterceptRequest/nativeFinished/nativeFailed/"
    ...
    const std::wstring error = ensureStarted(toWide(env, userDataDir), static_cast<int>(timeoutMs));
 ...
 */
WV_JNI(jlong, nativeRun)(JNIEnv* env, jobject, jstring url, jobjectArray headers, jstring script,
                         jlong token) {
    {
        std::lock_guard<std::mutex> lock(g_envMutex);
        if (!g_envDone || !g_envError.empty()) return -1;
    }
    WV_LOG("nativeRun: token=%lld url=%ls", static_cast<long long>(token), toWide(env, url).c_str());
    ...
    if (!runSync(
            [view, opt] {
                ...
                const HRESULT hr = g_env->CreateCoreWebView2Controller(hwnd, handler);
                handler->Release();
                if (FAILED(hr)) destroyView(hwnd);
            },
            5000)) {
        WV_LOG("nativeRun: 建窗口/控制器同步任务超时 (token=%lld)", static_cast<long long>(token));
    ...
    if (!hwnd) {
        WV_LOG("nativeRun: 宿主窗口没建起来 (token=%lld)", static_cast<long long>(token));
        return -1;
    }
    WV_LOG("nativeRun: handle=%p token=%lld", hwnd, static_cast<long long>(token));
 ...
 * `SwingPanel { Canvas() }` 里的 Canvas 变 `isDisplayable`（拿到 peer/HWND）之后，
 * 再调 `nativeViewAttach(handle, canvas)` 把它挂上去，由 `nativeViewFitToParent`
 * 负责尺寸。
 *
...
WV_JNI(jlong, nativeCreateView)(JNIEnv* env, jobject, jlong parentHwnd, jstring userAgent,
    ...
    if (!runSync(
            [view, opt] {
                ...
                if (!hwnd) {
                    WV_LOG("nativeCreateView: CreateWindowExW failed (%lu)", GetLastError());
                    return;
                }
                view->hwnd = hwnd;
                {
...
/**
 * 更新可见视图的尺寸（兼容入口；正常路径是 attach 之后的 `nativeViewFitToParent`）。
 *
 * 已经 attach 的视图尺寸**以父窗口客户区为准**，这里报来的值只在还没 attach 时
 * 当缓存用 —— 免得 AWT 和父窗口客户区因为 DPI 取整差一两个像素互相打架。
 */
WV_JNI(void, nativeSetViewBounds)(JNIEnv*, jobject, jlong handle, jint w, jint h) {
    const HWND hwnd = reinterpret_cast<HWND>(static_cast<intptr_t>(handle));
    ...
    postTask([hwnd, width, height] {
        ...
        if (view->ready && view->controller) view->controller->put_Bounds(view->pendingBounds);
    });
}
 ...
 * **AWT 线程**上做（组件 peer 归属那个线程），只读、不碰绘制；`SetParent` 之后的
 * 事全部投到 WebView 线程。
 *
 * 顺序不能反：必须先 `SetParent` 再改样式 —— 反过来在父链还没接上时把窗口改成
 * `WS_CHILD`，窗口会直接消失。改完样式 `ShowWindow`，此时才有东西可看。
 */
WV_JNI(void, nativeViewAttach)(JNIEnv* env, jobject, jlong handle, jobject awtComponent) {
    const HWND hwnd = reinterpret_cast<HWND>(static_cast<intptr_t>(handle));
...
WV_JNI(void, nativeSetViewVisible)(JNIEnv*, jobject, jlong handle, jboolean visible) {
    const HWND hwnd = reinterpret_cast<HWND>(static_cast<intptr_t>(handle));
    ...
    postTask([hwnd, v] {
        ...
        if (view->background || view->parent) ShowWindow(hwnd, v ? SW_SHOWNA : SW_HIDE);
        if (view->controller) view->controller->put_IsVisible(v);
    });
}

WV_JNI(void, nativeViewLoadUrl)(JNIEnv* env, jobject, jlong handle, jstring url, jobjectArray headers) {
    const HWND hwnd = reinterpret_cast<HWND>(static_cast<intptr_t>(handle));
    ...
    postTask([hwnd, u, h] {
        ...
        if (!h.empty() && view->env2) {
            ...
            if (SUCCEEDED(view->webview->QueryInterface(IID_ICoreWebView2_2,
                                                       reinterpret_cast<void**>(wv2.GetAddressOf())))) {
                ComPtr<ICoreWebView2WebResourceRequest> request;
                const std::wstring block = buildHeaderBlock(h);
                if (SUCCEEDED(view->env2->CreateWebResourceRequest(u.c_str(), L"GET", nullptr,
                                                                  block.c_str(), request.GetAddressOf())) &&
                    request && SUCCEEDED(wv2->NavigateWithWebResourceRequest(request.Get())))
                    return;
            }
        }
...
WV_JNI(void, nativeViewLoadHtml)(JNIEnv* env, jobject, jlong handle, jstring html) {
    const HWND hwnd = reinterpret_cast<HWND>(static_cast<intptr_t>(handle));
...
WV_JNI(void, nativeViewAction)(JNIEnv*, jobject, jlong handle, jint action) {
    ...
    postTask([hwnd, action] {
        ...
        switch (action) {
            case 0: view->webview->GoBack(); break;
            case 1: view->webview->GoForward(); break;
            case 2: view->webview->Reload(); break;
            case 3: view->webview->Stop(); break;
            case 4: view->webview->OpenDevToolsWindow(); break;
            default: break;
        }
    });
}

WV_JNI(void, nativeViewEvaluate)(JNIEnv* env, jobject, jlong handle, jstring script, jlong token) {
    const HWND hwnd = reinterpret_cast<HWND>(static_cast<intptr_t>(handle));
...
/**
 * 探针：在**另一条** STA + 消息泵的线程上，用「在 WebView 线程上建出来的 environment」
 * 建一个控制器。
 *
 * 动机：想把可见视图改成 wvbridge 那样**建在 AWT 线程上**（父子同线程，焦点链天然
 * 连通，就不需要 `AttachThreadInput` 那套），同时让后台视图留在 WebView 线程上
 * （脚本抓页面不该占用 UI 线程）。但两边必须共用**同一个 environment** —— cookie
 * 是按 user data folder 分的，两个 environment 就是两套登录态。
 *
 * 官方文档只写了「控制器必须建在有消息泵的 UI 线程上、所有调用都得在那条线程」，
...
struct ProbeControllerHandler final
    ...
    HRESULT STDMETHODCALLTYPE Invoke(HRESULT errorCode, ICoreWebView2Controller* controller) override {
        WV_LOG("probe: 跨线程 CreateCoreWebView2Controller 回调 hr=0x%08lX controller=%p",
               static_cast<unsigned long>(errorCode), static_cast<void*>(controller));
        g_probeHr.store(errorCode);
        // 探针不留东西：拿到就关掉。Close 的收尾要继续抽消息，所以下面还会泵一会儿。
...
void probeThreadMain() {
    ...
    {
        std::lock_guard<std::mutex> lock(g_envMutex);
        if (g_env && handler) hr = g_env->CreateCoreWebView2Controller(hwnd, handler);
    }
    if (handler) handler->Release();
    WV_LOG("probe: CreateCoreWebView2Controller 调用返回 hr=0x%08lX", static_cast<unsigned long>(hr));
    if (FAILED(hr)) {
        g_probeHr.store(hr);
        g_probeDone.store(true);
    }
    ...
    MSG msg{};
    while (!g_probeDone.load() && static_cast<int>(GetTickCount() - deadline) < 0) {
        while (PeekMessageW(&msg, nullptr, 0, 0, PM_REMOVE)) {
            TranslateMessage(&msg);
            DispatchMessageW(&msg);
        }
        if (!g_probeDone.load()) MsgWaitForMultipleObjects(0, nullptr, FALSE, 30, QS_ALLINPUT);
    }
    if (!g_probeDone.load()) {
 ...
 *      收尾工作永远没机会执行，`CoUninitialize()`（以及 DLL 卸载时的静态析构）就
 *      卡在那里，表现成线程退不掉。
 *   2. 再让消息泵退出：先 `WM_CLOSE` 掉宿主窗口，**兜底**再 `PostThreadMessageW(WM_QUIT)`
 *      —— 后者不依赖任何窗口存活，窗口已经被销毁过也不会失败。
 *
 * **等待必须是有界的**：环境一旦是坏的（实测过 WebView2 浏览器进程 `CHECK` 失败之后，
 * `controller->Close()` 能卡住十几秒甚至更久），无界 `join()` 会把 JVM 的 shutdown hook
 * 一起吊死 —— 进程退不掉，外面看到的就是 `Test Executor finished with non-zero exit
 * value 3`（Windows 上 `abort()` 的退出码）。超时就 detach：线程还挂在里面，但至少
 * 不再阻塞退出，也不会因为静态 `std::thread` 析构时还 joinable 而 `std::terminate`。
 */
WV_JNI(void, nativeStop)(WV_JNI_PARAMS) {
    if (g_thread.joinable()) {
        ...
        const bool tornDown = runSync(
            ...
            3000);
        if (!tornDown) WV_LOG("nativeStop: 拆视图的同步任务没跑完（3s），直接进退出流程");

        // ---- 2. 停泵 ----
        if (HWND dispatcher = g_dispatcher.load()) PostMessageW(dispatcher, WM_CLOSE, 0, 0);
        // 兜底：宿主窗口可能已经没了（WM_CLOSE 走的是窗口消息，窗口一销毁就再也投不进去）。
        // 线程消息不依赖窗口，只要消息泵还在 GetMessage 就一定能被抽到。
        if (const DWORD tid = g_threadId.load()) PostThreadMessageW(tid, WM_QUIT, 0, 0);
    }

    bool threadExited = true;
    if (g_thread.joinable()) {
        std::unique_lock<std::mutex> lock(g_threadMutex);
        threadExited = g_threadCv.wait_for(lock, std::chrono::seconds(5), [] { return g_threadExited; });
        lock.unlock();
        if (threadExited) {
            g_thread.join();
        } else {
            WV_LOG("nativeStop: WebView 线程 5s 没退出来（大概卡在关闭 WebView2 上），"
                   "detach 掉，不再阻塞进程退出");
            g_thread.detach();
