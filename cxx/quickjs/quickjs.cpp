...
Java_soko_ekibun_quickjs_QuickJS_initContext(JNIEnv *env, jclass, jobject ctx,
                                             jlong stack_size, jlong memory_limit,
                                             jlong timeout_ms) {
  JavaVM *javaVm;
  ...
  JS_SetInterruptHandler(rt, [](JSRuntime *rt, void *) -> int {
    auto opaque = (JSRuntimeOpaque *) JS_GetRuntimeOpaque(rt);
    if (opaque == nullptr || opaque->timeoutMs <= 0)
      return 0;
    auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::steady_clock::now() - opaque->evalStart).count();
    if (elapsed <= opaque->timeoutMs)
      return 0;
...
Java_soko_ekibun_quickjs_QuickJS_jsNewArrayBuffer(JNIEnv *env, jclass, jlong ctx,
  ...
  auto ret = (jlong) new JSValue(JS_NewArrayBufferCopy(
      (JSContext *) ctx, (const uint8_t *) elems, len));
...
extern "C"
JNIEXPORT void JNICALL
Java_soko_ekibun_quickjs_QuickJS_jsReleaseValue(JNIEnv *, jclass, jlong ctx, jlong obj) {
...
Java_soko_ekibun_quickjs_QuickJS_evaluate(JNIEnv *env, jclass, jlong ctx, jstring cmd,
  ...
  auto opaque = (JSRuntimeOpaque *) JS_GetRuntimeOpaque(JS_GetRuntime((JSContext *) ctx));
  if (opaque != nullptr) {
  ...
  auto ret = (jlong) new JSValue(JS_Eval((JSContext *) ctx,
                                        cmdChars,
                                        cmdLen,
                                        nameChars,
                                        flag));
 ...
 *
 * The object case is handled *here* rather than at each call site. That matters:
 * the recursive conversions (array elements, a promise's `then`) previously
 * called a scalar-only helper, so any object reached through them silently
 * became NULL. Keeping the dispatch in one place makes that impossible.
 */
 ...
 * that has to consult the persistent wrapper registry, and because that registry
 * pre-seeds `cache` -- the split keeps the scalar fast path above free of any
 * Java upcall.
 */
static jobject jsToJavaObject(JNIEnv *env, JSContext *ctx, JSValue obj,
                              std::unordered_map<void *, jobject> &cache) {
  ...
  // conversion. It is registered on the Kotlin side (see `Context.wrapperCache`)
  // and has to take precedence over the per-call `cache`, otherwise identity
  // would only hold *within* one traversal.
  auto ptr = JS_VALUE_GET_PTR(obj);
  jclass ctxClazz = env->GetObjectClass(opaque->thiz);
  auto persistent = env->CallObjectMethod(
      opaque->thiz, env->GetMethodID(ctxClazz, "peekWrapper", "(J)Ljava/lang/Object;"),
      (jlong) ptr);
  if (persistent != nullptr) {
    ...
    auto wrapper = env->CallObjectMethod(
        opaque->thiz, env->GetMethodID(ctxClazz, "reuseWrapper",
  ...
  auto remember = [&](jobject wrapper) -> jobject {
    cache[ptr] = wrapper;
    // Publish for later conversions. A fresh wrapper owns exactly one reference,
    // so it is registered here with the same handle the caller will later retire.
    env->CallVoidMethod(opaque->thiz, env->GetMethodID(ctxClazz, "registerWrapper",
  ...
  if (JS_IsFunction(ctx, obj)) {
    ...
    return remember(env->NewObject(clazz, init, (jlong) new JSValue(JS_DupValue(ctx, obj)),
                                   opaque->thiz));
...
Java_soko_ekibun_quickjs_QuickJS_jsToJava(JNIEnv *env, jclass, jlong ctx, jlong obj) {
  ...
  // jsToJava consumes the reference it is given (JS_GetProperty / JS_Call results
  // are handed over, not borrowed), so drop it here. The wrapper itself survives
...
Java_soko_ekibun_quickjs_QuickJS_jsCall(JNIEnv *env, jclass, jlong ctx, jlong obj,
  ...
  auto argvJs = (JSValue *) js_malloc((JSContext *) ctx, sizeof(JSValue) * (argc > 0 ? argc : 1));
  if (argvJs == nullptr) {
