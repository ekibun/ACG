...
class NativeWebViewHostTest {
    ...
    @Test
    fun subResourcesAreVisibleToInterceptor() = runBlocking {
        ...
        val hit = assertIs<WebViewTaskResult.Intercepted>(
            result,
    ...
    @Test
    fun scriptResultReflectsLoadedPage() = runBlocking {
        if (!webViewReady()) return@runBlocking
     ...
     */
    @Test
    fun cookiesAreSharedAcrossTasks() = runBlocking {
        if (!webViewReady()) return@runBlocking
        ...
        val wrote = guarded {
            loadBackgroundWebView(
                WebViewTask(
                    url = url("/page.html"),
                    script = "document.cookie = 'acg_probe=1; path=/; max-age=3600'; " +
     ...
     */
    @Test
    fun visibleViewEvaluateReturnsScriptResult() = runBlocking {
        if (!webViewReady()) return@runBlocking
        ...
        try {
            ...
            val json = NativeWebView.evaluateScript(
                handle,
    ...
    private fun com.sun.net.httpserver.HttpExchange.respond(contentType: String, body: ByteArray) {
