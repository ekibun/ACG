...
public object NativeWebView {
    ...
    private fun loadNativeLibraries() {
        ...
        val payload = NATIVE_FILES.associateWith { name ->
     ...
     */
    internal fun attachView(handle: Long, component: Component) =
     ...
     */
    internal fun setViewBounds(handle: Long, width: Int, height: Int) =
        nativeSetViewBounds(handle, width, height)

    internal fun setViewVisible(handle: Long, visible: Boolean) =
    ...
    internal fun loadUrl(handle: Long, url: String, headers: Map<String, String> = emptyMap()) =
        nativeViewLoadUrl(handle, url, headers.toFlatArray())

    internal fun loadHtml(handle: Long, html: String) = nativeViewLoadHtml(handle, html)

    internal fun viewAction(handle: Long, action: Int) = nativeViewAction(handle, action)
    ...
    internal fun evaluate(handle: Long, script: String, token: Long) =
        nativeViewEvaluate(handle, script, token)

    /** 注册一个视图事件监听；注册前的早期事件会被补发。 */
    internal fun addViewListener(handle: Long, listener: (what: Int, a: String?, b: String?) -> Unit) =
     ...
     */
    internal fun run(url: String, headers: Map<String, String>, script: String?, token: Long): Long {
    ...
    internal fun registerBackground(token: Long, sink: BackgroundSink) =
    ...
    internal interface BackgroundSink {
        /** 返回 true 表示已命中并收工。 */
        fun onIntercept(url: String, method: String, isForMainFrame: Boolean, headers: Map<String, String>): Boolean
    ...
    public class Callbacks {
        ...
        internal fun addBackground(token: Long, sink: BackgroundSink) {
        ...
        internal fun addViewListener(handle: Long, listener: (Int, String?, String?) -> Unit) {
        ...
        public fun nativeFinished(token: Long, json: String?) {
            background[token]?.onFinished(json)
        }

        public fun nativeFailed(token: Long, message: String) {
            background[token]?.onFailed(message)
        }

        public fun nativeViewEvent(handle: Long, what: Int, a: String?, b: String?) {
        ...
        public fun nativeScriptResult(handle: Long, token: Long, json: String?) {
    ...
    // ---- native ----
    private external fun nativeStart(callback: Any, userDataDir: String, timeoutMs: Int): String?
    ...
    private external fun nativeViewAttach(handle: Long, component: Component)
    private external fun nativeViewFitToParent(handle: Long)
    private external fun nativeSetViewBounds(handle: Long, width: Int, height: Int)
    private external fun nativeSetViewVisible(handle: Long, visible: Boolean)
    private external fun nativeViewLoadUrl(handle: Long, url: String, headers: Array<String>)
    private external fun nativeViewLoadHtml(handle: Long, html: String)
    private external fun nativeViewAction(handle: Long, action: Int)
    private external fun nativeViewEvaluate(handle: Long, script: String, token: Long)
